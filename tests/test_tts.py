...
async def main():
    ...
    print(f"  Audio: {len(result.audio_bytes)} bytes, ~{result.duration_ms:.0f}ms (est)")
    print("  [OK]")
    ...
    assert result2.duration_ms > result.duration_ms, "Longer text should estimate longer"
    print(f"  Audio: {len(result2.audio_bytes)} bytes, ~{result2.duration_ms:.0f}ms (est)")
    print("  [OK]")
    ...
    output_path = TEST_AUDIO_DIR / "tts_output_1.mp3"
